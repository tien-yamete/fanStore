...
void drawPortableFan(vec3 position, vec3 rotation, vec3 scale, bool enableInput)
{
    ...
    drawCube(vec3(0, 1.2f, -.5f), vec3(0), vec3(1, 1, 1), color3(0.2f, 0.8f, 0.2f));
    drawCube(vec3(0, 1.2f, -1.25f), vec3(0), vec3(.3f, .1f, .1f), color3(0.2f, 0.8f, 0.2f));

    
    
     auto matrix = cubeTransformMatrix(mat1 * Angel::Translate(0, 1.25f, -1.25f) * Angel::RotateX(90)
